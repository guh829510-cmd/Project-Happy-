...
(MIT). Lives in [`.claude/skills/remove-claude-marks/`](.claude/skills/remove-claude-marks/).
...
this PNG") or run `/remove-claude-marks`.
...
```bash
cd .claude/skills && zip -r ../../remove-claude-marks.zip remove-claude-marks
```

The zip must contain the `remove-claude-marks/` folder with `SKILL.md` at its
...
```bash
cp -r .claude/skills/remove-claude-marks ~/.claude/skills/
```
...
[`references/ethics.md`](.claude/skills/remove-claude-marks/references/ethics.md).
