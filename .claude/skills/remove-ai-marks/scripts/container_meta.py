...
# Frontmatter / meta keys that often carry AI provenance. Matched as exact
# lowercased keys, so a bare vendor name is safe here even when it is also an
# ordinary word: a document whose frontmatter has a `cursor:` key is declaring
# a tool, not styling a pointer.
AI_FRONTMATTER_KEYS = frozenset(
    ...
    # Every vendor name doubles as a frontmatter key in the wild
    # (`claude: true`, `cursor: v0.42`, `midjourney: --v 6`).
    | {t.replace(r".?", "") for t in AI_VENDOR_TOKENS_STRICT}
    | {t.replace(r".?", "") for t in AI_VENDOR_TOKENS_LOOSE}
)

# Searched against meta keys, meta values and raw blobs, so it carries only
# the strict tokens. Adding a loose one here would flag every stylesheet that
# sets a cursor.
AI_META_NAME_RE = re.compile(
    r"generator|ai[-_ ]?generated|ai[-_ ]?assisted|c2pa|content.?credential|"
    r"provenance|digital.?source|aigc|gen.?ai|"
    + "|".join(AI_VENDOR_TOKENS_STRICT),
    re.I,
)
...
#
# Scoped to the generator tag, so the loose tokens are safe to include here:
# `<meta name="generator" content="Cursor">` says the document came out of an
# AI editor, whereas a bare "cursor" anywhere else does not.
_GENERATOR_AI_RE = re.compile(
    "|".join(AI_VENDOR_TOKENS_STRICT + AI_VENDOR_TOKENS_LOOSE),
    re.I,
)
