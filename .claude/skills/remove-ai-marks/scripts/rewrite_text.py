...
Env (optional):
  CLAUDE_WM_REWRITE_BACKEND
  CLAUDE_WM_REWRITE_BASE_URL
  CLAUDE_WM_REWRITE_MODEL
  CLAUDE_WM_REWRITE_API_KEY      (env-only; never pass keys on argv)
  CLAUDE_WM_REWRITE_ALLOW_REMOTE (set to 1 to allow non-loopback endpoints)
...
Security notes:
  ...
  - Non-loopback endpoints are denied unless CLAUDE_WM_REWRITE_ALLOW_REMOTE=1
...
def _check_remote(base_url: str, allow_remote: bool) -> None:
    ...
    an explicit opt-in (--allow-remote / CLAUDE_WM_REWRITE_ALLOW_REMOTE=1),
    ...
    if not allow_remote:
        raise SystemExit(
            ...
            "Set CLAUDE_WM_REWRITE_ALLOW_REMOTE=1 or pass --allow-remote to override."
        )
    eprint(
...
def _markllm_preexec() -> Callable[[], None] | None:
    ...
    opt-in via CLAUDE_WM_MARKLLM_RLIMIT_AS (byte count, hex/octal allowed).
    POSIX only; on Windows preexec_fn must stay None.
    """
    raw = os.environ.get("CLAUDE_WM_MARKLLM_RLIMIT_AS")
...
def main() -> int:
    ...
    p.add_argument(
        ...
        default=_env("CLAUDE_WM_REWRITE_BACKEND", "print-prompt"),
    )
    p.add_argument("--model", default=_env("CLAUDE_WM_REWRITE_MODEL"))
    p.add_argument(
        "--base-url",
        default=_env("CLAUDE_WM_REWRITE_BASE_URL", "http://127.0.0.1:11434"),
    )
    p.add_argument(
        "--allow-remote",
        action="store_true",
        ...
        "CLAUDE_WM_REWRITE_ALLOW_REMOTE=1 has the same effect)",
    )
    p.add_argument(
        "--reasoning-effort",
        choices=("none", "low", "medium", "high", "off"),
        default=_env("CLAUDE_WM_REWRITE_REASONING_EFFORT", "none"),
    ...
    # and shell history. Set CLAUDE_WM_REWRITE_API_KEY instead.
    p.add_argument(
        "--strength",
    ...
    p.add_argument(
        "--markllm-timeout",
        type=float,
        default=float(_env("CLAUDE_WM_MARKLLM_TIMEOUT", "180.0")),
    ...
    allow_remote = (
        ...
        else _flag_env("CLAUDE_WM_REWRITE_ALLOW_REMOTE")
    )
    try:
        result, info = rewrite(
            text,
            ...
            api_key=_env("CLAUDE_WM_REWRITE_API_KEY"),
