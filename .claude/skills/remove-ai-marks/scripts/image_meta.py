...
_CTRLREGEN_RLIMIT_AS = int(os.environ.get("CLAUDE_WM_CTRLREGEN_RLIMIT_AS", str(32 << 30)))
_CTRLREGEN_RLIMIT_FSIZE = int(os.environ.get("CLAUDE_WM_CTRLREGEN_RLIMIT_FSIZE", str(2 << 30)))
...
# Vendor / product names distinctive enough to match anywhere: inside a
# metadata value, an XMP packet, or a raw blob, without colliding with
# ordinary prose or markup. Used to build both the byte hints below and the
# container-side regexes in container_meta.py.
#
# Regex fragments, so `.?` absorbs the separator variants vendors ship
# ("DALL-E" / "DALL·E" / "DALLE", "Stable Diffusion" / "stable-diffusion").
AI_VENDOR_TOKENS_STRICT: tuple[str, ...] = (
    r"chatgpt",
    r"openai",
    r"claude",
    r"anthropic",
...
# Real AI generators whose names are also ordinary English words or common
# identifiers ("cursor" in CSS, "flux" in physics, "llama" the animal). A blob
# scan for these produces false positives, so they are matched only where the
# surrounding structure already declares a generator: a <meta name="generator">
# content attribute, or a frontmatter key. Never in free-text scans.
AI_VENDOR_TOKENS_LOOSE: tuple[str, ...] = (
    r"cursor",
    r"windsurf",
    r"codeium",
    r"tabnine",
...
# Literal spellings for the byte scanner. Image metadata is written by image
# and text generators, so the code-editor assistants (Cursor, Windsurf,
# Codeium) are deliberately absent: they mark documents, not pixels, and their
# names are too common to risk against raw image bytes.
_AI_VENDOR_LITERALS: tuple[str, ...] = (
    "ChatGPT",
    "OpenAI",
    "Claude",
    "Anthropic",
...
AI_META_HINTS = (
    ...
    b"GenAI",
) + tuple(name.encode("utf-8") for name in _AI_VENDOR_LITERALS)
