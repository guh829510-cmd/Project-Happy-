...
## Metadata-level tools (no model-level watermark)

A larger group of tools leaves no statistical mark but does stamp its name into
file metadata: a `<meta name="generator">` tag, a YAML frontmatter key, an XMP
packet, or a PNG text chunk. That is Layer A / container territory, and a strip
removes it completely, unlike the sampling marks above.

| Tool | Where the name usually lands |
| --- | --- |
| ChatGPT, Claude, Gemini, Grok, DeepSeek, Perplexity | Frontmatter key, generator meta, export headers |
| GitHub Copilot, Cursor, Windsurf, Codeium, Tabnine, Replit | Generator meta and frontmatter in exported or scaffolded docs |
...
Matching runs in two tiers, because several of these names are ordinary words.
Distinctive tokens (`chatgpt`, `midjourney`, `synthid`) match anywhere, including
raw metadata blobs. Ambiguous ones (`cursor`, `flux`, `llama`, `firefly`, `grok`,
`sora`, `jasper`, `mistral`) match only inside a structure that already declares a
generator, so CSS that sets `cursor: pointer` and an article about the Gemini
program stay clean. The lists are `AI_VENDOR_TOKENS_STRICT` and
`AI_VENDOR_TOKENS_LOOSE` in `service/scripts/image_meta.py`; `tests/test_vendor_coverage.py`
pins both halves.

A hit here means the tool wrote its name into the file. It is not evidence about
how much of the content the tool produced, and its absence is not evidence that
no tool was used.
